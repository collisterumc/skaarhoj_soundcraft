...
Assume the claim in front of you is wrong and go looking for evidence. Report "verified,
no issues" only if you genuinely tried to disprove it and can say what you tried.
...
- **State that outlives its validity.** After a failure or reset, is anything still serving
  values from before?
- **The test's own blind spots.** If a mock or fixture skips a documented deviation, it
  hides exactly the bug that deviation predicts. Read the test doubles before you trust a
  green run.

## How to verify

Prefer running the thing over reasoning about it. Build it, run it, feed it the bad input,
read the actual output. Wrong claims survive review when nobody runs the code.
...
`reference/` is a useful cross-check on protocol questions, but it documents behavior; it
does not measure it. Hardware results outrank it.
...
If you could not test something, say so and say why. Naming an untested area is worth
